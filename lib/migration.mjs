...
export async function usMigration() {
  const out = { headline: null, stock: [], flow: [], protection: [], errors: [] };
  ...
  // The plain statement: how many people live here, how many were born abroad,
  // and what share that is. Deliberately pinned to a single year — total
  // population runs a year ahead of the migrant-stock estimate, and 15.2% of a
  // 2025 population does not reconcile with a 2024 stock. Better to state one
  // consistent year than three figures that do not add up.
  try {
    const pop = await wbSeries('USA', 'SP.POP.TOTL');
    const stock = out.stock.find((m) => m.metric === 'migrant_stock');
    const share = out.stock.find((m) => m.metric === 'migrant_share');
    if (stock && pop.points.length) {
      const year = Number(stock.period);
      const match = pop.points.find((q) => q.year === year);
      if (match) {
        out.headline = {
          year,
          period_type: 'calendar year',
          population: match.value,
          foreignBorn: stock.value,
          // The publisher's own share for that year, not a division done here.
          share: share && share.period === stock.period ? share.value : null,
          source: { ...WB_SOURCE, url: 'https://data.worldbank.org/indicator/SP.POP.TOTL' },
        };
      }
    }
  } catch (err) {
    out.errors.push(`population: ${err.message}`);
  }
