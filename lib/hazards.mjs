...
//
// Wildfire is deliberately absent. NASA FIRMS is the right source and it
// requires a MAP_KEY; rather than substitute something weaker, the UI reports
// it as unavailable with that reason.
...
export async function activeHazards() {
  ...
  return {
    ...
    // Stated rather than silently omitted: this needs a credential we do not have.
    unavailable: [{
      kind: 'wildfire',
      reason: 'NASA FIRMS requires a MAP_KEY. Register free at '
        + 'https://firms.modaps.eosdis.nasa.gov/api/area/ and set FIRMS_MAP_KEY.',
    }],
  };
}
