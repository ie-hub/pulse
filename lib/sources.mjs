...
export const SOURCES = [
  ...
  { place: 'indiana', id: 'iedc',         lane: 'official', outlet: 'Indiana Econ Dev', kind: 'iedc',
    // Scraped, not a feed — IEDC publishes none. This is the endpoint its own
    // pager posts to; `pages` is how far back to walk, ten stories per page.
    url: 'https://iedc.in.gov/events/news/Paging/',
    // Twelve pages reaches back about two years. Five was enough when it was
    // set and then was not: IEDC keeps publishing, so a fixed depth is a
    // sliding window, and the article this was checked against had dropped off
    // the end of it. Depth is what makes the archive searchable rather than
    // just recent. The global government rail still shows only its newest few.
    pages: 12 },
